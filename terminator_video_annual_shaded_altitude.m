function terminator_video_annual_shaded_altitude(Y)
%Animate the terminator over the course of a year, for 12:00 UT every day%
%Author: Dr. T.S. Kostadinov, 2015-2020, animation added 2019-2020
%
%Inputs:
%Option 1: No inputs, assume user wants year 2020
%Option 2: Enter the year desired (assumes Gregorian calendar)
% Y - year
...
if isleap_vectorized(Y)
    dd = 1:366;
else
    dd = 1:365;
end
MMDD = dofyear2date_v2020(Y,dd);
H = 12; %At 12 UT every day of the year

%Set up figure
f1h = figure(1);
clf;
set(f1h,'Name','Day/Night Across Earth Over a Year, at 12:00 UT','NumberTitle','off','Units','normalized','OuterPosition',[0.15 0.1 .85 .85]);
...
for m = 1:size(MMDD,1)
    ...
    JD = date2jd_vec(Y,MMDD(m,1),MMDD(m,2),H,0,0,'G'); %Julian day, input date/time has to be in UT!
    ...
    if strncmp(mver.Version,'9.9',3) %new subtitle command will not work in recent older versions
        ...
        subtitle([datestr(datenum(Y,MMDD(m,1),MMDD(m,2),H,0,0),31),' UT; \delta Sun = ',sprintf('%02.2f',dec_Sun),'^o' ]);
    else
        title(['Terminator (red line), subsolar (red star) and antisolar (black X) points, and solar altitude (deg) (color scale) \newline',...
            '                                                    ',...
            datestr(datenum(Y,MMDD(m,1),MMDD(m,2),H,0,0),31),' UT; \delta Sun = ',sprintf('%02.2f',dec_Sun),'^o' ]);
    end
